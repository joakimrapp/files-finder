...
const find = ( absolutepath, rule ) => log
	...
	.promise;
module.exports = ( scanpath, rule = '*', steps = 0 ) => find( path.isAbsolute( scanpath ) ? scanpath :
	path.resolve( path.dirname( require( 'stack-trace' ).get()[ 1 + steps ].getFileName() ), scanpath ), rule );
